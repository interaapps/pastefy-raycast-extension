...
export default function Command() {
  ...
  const { isLoading, data: folders } = usePromise<any>(
    () => async () => {
      try {
        ...
        const folders: { id: any; path: string; }[] = []

        const addFolders = (folder: { id: any; name: string; children: [] }, prefix = '') => {
  ...
  async function handleSubmit(values: Values) {
    ...
    if (paste.encrypted) {
      currentPassword = values.encrypted && !password ? Math.random().toString(36).substring(3) + Math.random().toString(36).substring(3) : password;
  ...
  return (
    ...
    >
      <Form.TextField onChange={setTitle} id="title" title="Title" placeholder="Title.txt (.md for markdown)" defaultValue="" />
      <Form.TextArea id="content" enableMarkdown={title?.endsWith('.md')} title="Code input" placeholder="Enter multi-line text"  />
      <Form.Separator />
      ...
      { folders === null 
        ...
        : <Form.Dropdown isLoading={isLoading} id="folder" title="Folder">
            ...
            {folders?.map((folder: any) => <Form.Dropdown.Item key={folder.id} value={folder.id} title={folder.path} />)}
          </Form.Dropdown> 
      }
